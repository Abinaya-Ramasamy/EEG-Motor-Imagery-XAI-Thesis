...
path.append('/projects/sciences/computing/ramab620/eegxai-temp-rnn/utils')
...
data_link = f'/projects/sciences/computing/ramab620/eegxai-temp-rnn/dataset/{snr}_{domain}'
model_link = f'/projects/sciences/computing/ramab620/eegxai-temp-rnn/dataset/{model_name}/'
result_link = f'/projects/sciences/computing/ramab620/eegxai-temp-rnn/result/{model_name}/'
...
for fold_id in range(n_fold): 
    ...
    print("X_test shape before dataloader:", X_test.shape)
    #gt_test = swapaxes(gt_test, 1, 2)
    
    X_test_gen = DataGenerator(X_test, y_test)
    X_test_loader = DataLoader(X_test_gen, batch_size=batch_size)

    xb, yb = next(iter(X_test_loader))
