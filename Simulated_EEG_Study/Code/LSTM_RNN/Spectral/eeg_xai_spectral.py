...
file_path = '../dataset/sim_db_spectral1.set'
...
from eeg_train import GetDataFold_stratified, GetDataLoader, TrainingModel, EvalModel, AdjustDimenson
from eeg_models import SelectModel
from eeg_dbf import LblEncoding
#from tensorflow.keras.layers import ConvLSTM2D 
#from tensorflow.keras.models import Sequential
#from tensorflow.keras.layers import LSTM, Dense, Flatten, TimeDistributed, Conv2D
# Parameter setup
#parser = ArgumentParser()
#parser.add_argument('--domain', type=str, required=False, choices=['temporal', 'spectral', 'spatial'], default="temporal")
...
os.makedirs(model_link, exist_ok=True)
...
try :
    ...
    labels = labels - labels.min()   #  makes classes start at 0 (0..n_class-1)
    print("Unique labels after fix:", np.unique(labels))
...
if random_label :
    ...
    random_labels_encoded = random_labels.astype(int)
...
for fold_id in range (n_fold):
    ...
    if random_label :
        ...
        #X_train, y_train, X_val, y_val, X_test, y_test = GetDataFold_stratified(eeg_data, random_labels_encoded, n_fold, fold_id, generator_size)
        X_train, y_train, X_val, y_val, X_test, y_test = GetDataFold_stratified(
            eeg_data, random_labels_encoded, n_fold, fold_id, seed=42
        )
       ...
       # X_train, y_train, X_val, y_val, X_test, y_test = GetDataFold(eeg_data, labels_encoded, n_fold, fold_id, generator_size)
        #X_train, y_train, X_val, y_val, X_test, y_test = GetDataFold_stratified(eeg_data, labels_encoded, n_fold, fold_id, generator_size)
        X_train, y_train, X_val, y_val, X_test, y_test = GetDataFold_stratified(
            eeg_data, labels_encoded, n_fold, fold_id, seed=42
        )
    ...
    y_train = to_class_index(y_train)
    y_val   = to_class_index(y_val)
    y_test  = to_class_index(y_test)

    print("y_train shape:", y_train.shape, "unique:", np.unique(y_train))


        # X_* shape: (N, C, T)
    mu  = X_train.mean(axis=(0, 2), keepdims=True)              # per-channel mean
    std = X_train.std(axis=(0, 2), keepdims=True) + 1e-8        # per-channel std
   ...
   # X_train, X_test, X_val = AdjustDimenson(X_train, X_test, X_val)
    if model_name == "LSTM":
        # Keep spectral data as (N, 62, 250)
        print("X_train shape for LSTM:", X_train.shape)
        print("X_val shape for LSTM:", X_val.shape)
        print("X_test shape for LSTM:", X_test.shape)
       #X_train = swapaxes(X_train, 1, 2)  # (N, T, C)
       #X_val   = swapaxes(X_val, 1, 2)
...
# Show model summary
#print(f'\nModel summary:')
#summary(train_model, (n_sample, n_chan))
#np.save(info_path.replace(".xlsx", "_fold_history.npy"), np.array(fold_history))
print("\nModel summary:")
print(train_model)
