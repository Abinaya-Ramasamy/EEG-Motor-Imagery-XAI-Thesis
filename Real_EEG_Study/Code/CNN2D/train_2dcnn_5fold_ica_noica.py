...
def train_one_fold(X_train_full, y_train_full, X_test, y_test, fold_num, seed, epochs=30):
    ...
    # Reshape for 2D CNN: (N, 1, channels, time)
    X_train = X_train[:, np.newaxis, :, :]
    X_val = X_val[:, np.newaxis, :, :]
    X_test = X_test[:, np.newaxis, :, :]

    print("2D CNN X_train shape:", X_train.shape)
    print("2D CNN X_val shape:", X_val.shape)
    print("2D CNN X_test shape:", X_test.shape)
    ...
    set_seed(seed)
    model = TwoDCNN(num_classes=2).to(device)
...
def run_5fold(data_path, save_folder, config_name, seed):
    ...
    print("\n===================================")
    print(f"Running TwoDCNN 5-Fold: {config_name}")
    print(f"Seed: {seed}")
    ...
    save_path = os.path.join(save_folder, f"twodcnn_5fold_results_seed_{seed}.xlsx")
...
for seed in SEEDS:
    ...
    run_5fold(
        ica_file,
        os.path.join(result_dir, f"TwoDCNN_2class_ICA_5fold_seed_{seed}"),
        "ICA",
        seed
    )
    ...
    run_5fold(
        noica_file,
        os.path.join(result_dir, f"TwoDCNN_2class_NoICA_5fold_seed_{seed}"),
        "NoICA",
        seed
    )