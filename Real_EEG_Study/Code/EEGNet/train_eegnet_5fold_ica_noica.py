...
def run_5fold(data_path, save_folder, config_name, seed):
    ...
    print("\n===================================")
    print(f"Running EEGNet 5-Fold: {config_name}")
    print(f"Seed: {seed}")
...
# remove model from result before saving to DataFrame
        del result["model"]
    ...
    save_path = os.path.join(save_folder, f"eegnet_5fold_results_seed_{seed}.xlsx")
    ...
    # SAVE MODEL (ADD THIS)
    # -----------------------------
    model_save_path = os.path.join(save_folder, f"eegnet_model_seed_{seed}.pth")
...
for seed in SEEDS:
    ...
    run_5fold(
        ica_file,
        os.path.join(result_dir, f"EEGNet_2class_ICA_5fold_seed_{seed}"),
        "ICA",
        seed
    )
    ...
    run_5fold(
        noica_file,
        os.path.join(result_dir, f"EEGNet_2class_NoICA_5fold_seed_{seed}"),
        "NoICA",
        seed
    )