...
class CNN1D(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.features = nn.Sequential(
            nn.Conv1d(22, 32, kernel_size=7, padding=3),
            nn.ReLU(),
            nn.MaxPool1d(2),
...
def run_5fold(data_path, save_folder, config_name, seed):
    ...
    print("\n===================================")
    print(f"Running CNN1D 5-Fold: {config_name}")
    print(f"Seed: {seed}")
    ...
    save_path = os.path.join(save_folder, f"cnn1d_5fold_results_seed_{seed}.xlsx")
...
for seed in SEEDS:
    ...
    run_5fold(
        ica_file,
        os.path.join(result_dir, f"CNN1D_2class_ICA_5fold_seed_{seed}"),
        "ICA",
        seed
    )
    ...
    run_5fold(
        noica_file,
        os.path.join(result_dir, f"CNN1D_2class_NoICA_5fold_seed_{seed}"),
        "NoICA",
        seed
    )